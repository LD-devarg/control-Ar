...
def crear_tarea_periodica(apps, schema_editor):
    ...
    PeriodicTask.objects.get_or_create(
        name="sync_pauta_kpi_15m",
        defaults={
            "interval": intervalo,
            "task": "apps.pauta.tasks.sync_pauta_kpi_15m",
            "enabled": True,
        },
    )
...
def borrar_tarea_periodica(apps, schema_editor):
    PeriodicTask = apps.get_model("django_celery_beat", "PeriodicTask")
    PeriodicTask.objects.filter(name="sync_pauta_kpi_15m").delete()


class Migration(migrations.Migration):
    dependencies = [
        ("pauta", "0004_rendimientopautadiario"),
