...
from apps.empresas.permissions import is_admin, is_admin_organizacional
...
def validar_creacion_usuario(usuario_creador, grupo_nuevo_usuario):
    ...
    if is_admin_organizacional(usuario_creador):
        if grupo_nuevo_usuario.name not in ["Admin", "Operador", "Pauta"]:
            raise ValidationError("Admin Organizacional solo puede crear Admin, Operador o Pauta.")
        return True
    ...
    if nuevo_grupo:
        ...
        if not usuario_modificador.is_superuser:
            if is_admin_organizacional(usuario_modificador):
                if nuevo_grupo.name not in ["Admin", "Operador", "Pauta"]:
                    raise ValidationError("Admin Organizacional solo puede asignar Admin, Operador o Pauta.")
            elif nuevo_grupo.name not in ["Operador", "Pauta"]:
